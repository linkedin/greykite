...
from greykite.sklearn.estimator.silverkite_multistage_estimator import SilverkiteMultistageEstimator
from greykite.sklearn.estimator.silverkite_multistage_estimator import SilverkiteMultistageModelConfig
...
def params():
    params = dict(
        ...
        freq="H",
        model_configs=[
            SilverkiteMultistageModelConfig(
                train_length="30D",
                fit_length="30D",
                agg_func="mean",
                agg_freq="D",
...
def test_silverkite_multistage_model_config():
    """Tests the default parameters in ``SilverkiteMultistageModelConfig``."""
    config = SilverkiteMultistageModelConfig()
...
def test_set_up(params):
    """Tests the set up of ``SilverkiteMultistageEstimator``."""
    # Instatiation.
    model = SilverkiteMultistageEstimator(**params)
...
def test_get_freq_col(params):
    model = SilverkiteMultistageEstimator(**params)
    model.time_col_ = "ttt"
    freq_col = model._get_freq_col(freq="D")
    assert freq_col == "ttt__D"


def test_get_non_time_cols(params):
    model = SilverkiteMultistageEstimator(**params)
    model.time_col_ = "ttt"
    columns = ["ttt", "ttt__D", "ttt__5T", "ttt_H", "value", "reg"]
...
def test_add_agg_freq_cols(params, hourly_data_with_reg):
    ...
    assert list(df.columns) == [cst.TIME_COL, cst.VALUE_COL, "regressor",
                                f"{cst.TIME_COL}__D", f"{cst.TIME_COL}__H"]
    assert df[f"{cst.TIME_COL}__D"].unique().shape[0] == 7 * 8  # data has 7 weeks.
    assert df[f"{cst.TIME_COL}__H"].unique().shape[0] == df.shape[0]  # hourly is the original freq.
...
def test_aggregate_values(params, hourly_data_with_reg):
    ...
    df_agg = model._aggregate_values(
        df=df[[f"{cst.TIME_COL}__D", cst.VALUE_COL, "regressor"]],
        agg_freq="D",
        agg_func=np.nanmean
    )
...
def test_train_and_predict(params, hourly_data_with_reg):
    ...
    # checks values-
    assert model.fit_df[f"{cst.VALUE_COL}__D"].dropna().shape[0] >= 30  # daily training size
    assert model.fit_df[f"{cst.VALUE_COL}__H"].dropna().shape[0] >= 24 * 7  # hourly training size
    assert model.fit_df[f"{cst.PREDICTED_COL}__D"].dropna().shape[0] >= 30  # daily fit size
    assert model.fit_df[f"{cst.PREDICTED_COL}__H"].dropna().shape[0] >= 30  # daily fit size
...
def test_same_agg_freq(params):
    params["model_configs"][1].agg_freq = "D"
    model = SilverkiteMultistageEstimator(**params)
    with pytest.raises(
            ValueError,
            match="Models from different stages should have different aggregation"):
        model._initialize()
